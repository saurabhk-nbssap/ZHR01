...
*&---------------------------------------------------------------------*
*----------------------------------------------------------------------*
* OBJECT DESCRIPTION: Gratuity Details Report
...
REPORT  z6hr036r_gratu_balances.
...
*      WA_FINAL-LIMIT = 180.
*    ENDIF.
    SELECT SINGLE * FROM  pa0185 INTO wa_pa0185 WHERE pernr EQ wa_final-pernr
                                                  AND subty EQ '03'
                                                  AND begda LE sy-datum
                                                  AND endda GE sy-datum.
    IF sy-subrc EQ 0.
...
FORM fieldcat1 USING p_it_st_fieldcat TYPE slis_t_fieldcat_alv.
  ...
  ls_fieldcat-seltext_m = 'Gratuity Number'.
  ls_fieldcat-seltext_l = 'Gratuity Number'.
  ls_fieldcat-seltext_s = 'Gratuity Number'.
  ls_fieldcat-ddictxt = 'M'.
  ls_fieldcat-ref_fieldname = 'ICNUM'.
  ls_fieldcat-ref_tabname = 'PA0185'.
...
*----------------*
  wa_header-typ = 'A'.
  wa_header-info = 'Employee Grattuity Details'.
