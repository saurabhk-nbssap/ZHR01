class Z6HR006C_LEAVE_REQ_VALN definition
  public
  final
  create public .

*"* public components of class Z6HR006C_LEAVE_REQ_VALN
...
public section.
  ...
  methods CONSTRUCTOR .
protected section.
*"* protected components of class CL_PT_ARQ_REQ_EXIT
*"* do not include other source files here!!!
private section.
*"* private components of class CL_PT_ARQ_REQ_EXIT
*"* do not include other source files here!!!
...
CLASS Z6HR006C_LEAVE_REQ_VALN IMPLEMENTATION.
...
*              WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4.    "Note983558
      ELSE.                                                 "Note983558
        IF wa_is_authorized EQ 'X'.                         "Note983558
* At least one IT0130 authorization exists --> no further   "Note983558
* checks are required                                       "Note983558
          EXIT.                                             "Note983558                                               "Note983558
        ENDIF.                                              "Note983558
      ENDIF.                                                "Note983558
    ELSE.                                                   "Note983558
* No record for test procedure found --> no further checks required
      wa_is_authorized = 'X'.                               "Note983558
      EXIT.                                                 "Note983558
    ENDIF.                                                  "Note983558
...
* XRK 20060103 Note912250 Disable authority check for simulation
  ...
  DATA: l_selcat      TYPE tim_tmw_selcattype_entry,
     ...
     I_DAY_PSP           TYPE TABLE OF PDSPPSP INITIAL SIZE 0,
        ...
        l_556q       TYPE t556q,
        l_538t       TYPE t538t,
         ET_REQUEST_LIST     TYPE PTREQ_REQUEST_TAB,
     EX_MESSAGES1         TYPE PTARQ_UIA_MESSAGES_TAB,
     EX_COMMANDS         TYPE PTARQ_UIA_COMMAND_TAB,
     WA_LIST     TYPE PTREQ_ITEMS_TAB_FLAT,
     IT_2001     TYPE PTARQ_P2001_TAB,
     WA_2001     TYPE P2001,
     IA_2001     TYPE TABLE OF P2001,
     WL_2001     TYPE P2001,
  ...
  DATA:
     ...
     LIST           TYPE ptarq_reqlist_tab_flat.
  DATA:
  ...
  DATA:
    constraints   TYPE ptarq_tconstr.
  CLASS:
    cl_pt_arq_const DEFINITION LOAD.
    ...
    ELSE.
      CALL METHOD cl_pt_arq_badi=>convert_attabs_to_blop_del
        ...
        IMPORTING
          ex_blop_tab   = lt_blop_temp.
    ENDIF.
    READ TABLE lt_blop_temp INDEX 1 INTO blop_wa TRANSPORTING control-recuid control-beguz control-enduz.
    attabs_temp-tmw_recuid = blop_wa-control-recuid.

    MODIFY lt_match_tab FROM attabs_temp.
    blop_wa-control-beguz = '090000'.
        blop_wa-control-enduz = '130000'.
...
*   authority check during simulation                               "INS Note1043502
    ...
    MODIFY lt_blop_temp INDEX 1 FROM blop_wa                        "INS Note1043502
                        TRANSPORTING control-authstat control-beguz control-enduz.              "INS Note1043502
...
*      READ TABLE attabs_deduct_wa-abs_attribs INDEX 1 INTO l_p2001.
*    endif.
  clear wa_abs_attribs.
  read table lt_match_tab into attabs_temp index 1.
  if sy-subrc eq 0.
    move attabs_temp-abs_attribs to ia_2001.
    read table ia_2001 into wa_abs_attribs index 1.
  endif.
                ...
                IF   wa_abs_attribs-BEGDA LT st_0019-TERMN.
                  ...
                  CALL METHOD MESSAGE_HANDLER->ADD_MESSAGE
                    EXPORTING
                      ...
                      IM_CL         = 'ZHR01'                                         "Message class that you created
                      IM_NUMBER     = '001'                                         "Message no
                      IM_PAR1       = 'Leave request can not be allowed till 11 months'
                      IM_PAR2       = 'of joining '          "Method name as in your impl.
                      IM_PAR3       = ' '
                      IM_CONTEXT    = ''
                      IM_SUBCONTEXT = ''
                      IM_CLASSNAME  = 'CL_PT_ARQ_BLOP_ADAPTER'
                ...
                IF wa_abs_attribs-BEGDA LT st_0019-TERMN.
                  CALL METHOD MESSAGE_HANDLER->ADD_MESSAGE
                    EXPORTING
                      ...
                      IM_CL         = 'ZHR01'                                         "Message class that you created
                      IM_NUMBER     = '001'                                         "Message no
                      IM_PAR1       = 'Leave Request Not Allowed'
                      IM_PAR2       = ' '          "Method name as in your impl.
                      IM_PAR3       = ' '
                      IM_CONTEXT    = ''
                      IM_SUBCONTEXT = ''
                      IM_CLASSNAME  = 'CL_PT_ARQ_BLOP_ADAPTER'
                ...
                IF wa_abs_attribs-BEGDA LT st_0019-TERMN.
                  CALL METHOD MESSAGE_HANDLER->ADD_MESSAGE
                    EXPORTING
                      ...
                      IM_CL         = 'ZHR01'                                         "Message class that you created
                      IM_NUMBER     = '001'                                         "Message no
                      IM_PAR1       = 'Leave Request Not Allowed till 6 months'
                      IM_PAR2       = 'of joining '          "Method name as in your impl.
                      IM_PAR3       = ' '
                      IM_CONTEXT    = ''
                      IM_SUBCONTEXT = ''
                ...
                IF wa_abs_attribs-BEGDA LT st_0019-TERMN.
                  CALL METHOD MESSAGE_HANDLER->ADD_MESSAGE
                    EXPORTING
                      ...
                      IM_CL         = 'ZHR01'                                         "Message class that you created
                      IM_NUMBER     = '001'                                         "Message no
                      IM_PAR1       = 'Leave Request Not Allowed During Probation'
                      IM_PAR2       = 'of joining '          "Method name as in your impl.
                      IM_PAR3       = ' '
                      IM_CONTEXT    = ''
                      IM_SUBCONTEXT = ''
...
*    APPEND wa_abs_attribs-PERNR TO pernr_tab.
*
*
**  DATA:
**    num_of_dates      TYPE i.
...
*      sel_date+4(4) = '0101'.
*    ELSE.
**    M  heute minus 1 Monat
**    B  Beginn des Monats
**    7  heute minus 1 Woche
**    W  Beginn der Woche
**    Y  heute minus 1 Jahr
**    S  Beginn des Jahres
*
*      CASE constraints-defaults_list.
*        WHEN cl_pt_arq_const=>c_deflist_month.
*
*          sel_date = sy-datum.
*          sel_date+4(2) = sel_date+4(2) - 1.
*
*          CALL METHOD cl_pt_req_utils=>get_valid_date
*            EXPORTING
*              im_date      = sel_date
*              im_direction = cl_pt_req_const=>c_direction_past
*            CHANGING
*              ex_date      = sel_date.
*
*
*        WHEN cl_pt_arq_const=>c_deflist_month_start.
*          sel_date = sy-datum.
*          sel_date+6(2) = '01'.
*
*        WHEN cl_pt_arq_const=>c_deflist_week.
*          sel_date = sy-datum - 7.
*
*        WHEN cl_pt_arq_const=>c_deflist_week_start.
*
**######### später weiteres Customizing auswerten! Der erste Wochentag
**######### ist eigentlich einstellbar, da regional unterschiedlich
*          CALL METHOD cl_pt_req_utils=>get_first_week_day
*            EXPORTING
*              im_refdate        = sy-datum
*              im_cust_first_day = custo_first_day
*            IMPORTING
*              ex_first_week_day = sel_date.
*
*        WHEN cl_pt_arq_const=>c_deflist_year.
*
*          sel_date = sy-datum.
*          sel_date(4) = sel_date(4) - 1.
*
*          CALL METHOD cl_pt_req_utils=>get_valid_date
*            EXPORTING
*              im_date      = sel_date
...
*              ex_date      = sel_date.
*
*        WHEN cl_pt_arq_const=>c_deflist_year_start.
*
*          sel_date = sy-datum.
*          sel_date+4(4) = '0101'.
*      ENDCASE.
...
*          im_selection_begin = SEL_DATE
*          im_selection_end   = '99991231'
*          im_read_it_data    = 'X'
*        IMPORTING
*          ex_all_my_reqs     = LIST.
  ...
  IF NOT P2001_TAB IS INITIAL.
    ia_2001 = p2001_tab.
    delete ia_2001 where begda eq wa_abs_attribs-begda
                     and endda eq wa_abs_attribs-endda
                     and subty eq wa_abs_attribs-subty.
*        LOOP AT P2001_TAB  INTO WA_2001.
*
*
*            MOVE WA_2001 TO WL_2001.
*            APPEND WL_2001 TO IA_2001.
*            CLEAR WL_2001.
*
*        ENDLOOP.
...
*      im_status_range     = status_range
**      im_selector         = l_selector
*      im_sel_only_current = cl_pt_req_const=>c_true
*      im_refresh          = 'X'
*    IMPORTING
*      ex_result_tab       = eT_request_list
*    EXCEPTIONS
*      actor_not_found     = 1
*      OTHERS              = 2.
...
*         WITH SY-MSGV1 SY-MSGV2 SY-MSGV3 SY-MSGV4.
      ...
      LOOP AT IA_2001  INTO WL_2001 where   ( subty EQ  'PL' OR SUBTY  EQ 'SL' or subty eq 'CL'  OR SUBTY EQ 'CL1' OR SUBTY EQ 'CL2' )
      ...
      IF SY-SUBRC EQ 0.
        CALL METHOD MESSAGE_HANDLER->ADD_MESSAGE
          EXPORTING
            ...
            IM_CL         = 'ZHR01'                                         "Message class that you created
            IM_NUMBER     = '001'                                         "Message no
            IM_PAR1       = 'CL Can not be combined With PL / SL'
            IM_PAR2       = ' '          "Method name as in your impl.
            IM_PAR3       = ' '
            IM_CONTEXT    = ''
            IM_SUBCONTEXT = ''
            IM_CLASSNAME  = 'Z6HR006C_LEAVE_REQ_VAL'
            IM_METHODNAME = 'MESSAGE'.
      ...
      IF SY-SUBRC EQ 0.
        CALL METHOD MESSAGE_HANDLER->ADD_MESSAGE
          EXPORTING
            ...
            IM_CL         = 'ZHR01'                                         "Message class that you created
            IM_NUMBER     = '001'                                         "Message no
            IM_PAR1       = 'Leave can not be applied'
            IM_PAR2       = ' '          "Method name as in your impl.
            IM_PAR3       = ' '
            IM_CONTEXT    = ''
            IM_SUBCONTEXT = ''
...
*          MESSAGE E016(RP) WITH 'CL Can not be combined With PL / SL ' .
      ...
      LV_PDATE = wa_abs_attribs-BEGDA.
      LOOP AT IA_2001  INTO WL_2001 where  ( SUBTY EQ 'CL1' OR SUBTY EQ 'CL2' OR SUBTY EQ 'SL1' OR SUBTY EQ 'SL2')
                                                     AND begda EQ LV_PDATE.
...
*      ENDIF.
    ...
    MOVE-CORRESPONDING attabs_temp TO attabs_deduct_wa.
    IF NOT attabs_deduct_wa-abs_attribs IS INITIAL.
      read table attabs_deduct_wa-abs_attribs into wa_abs_attribs index 1.
      if  wa_abs_attribs-subty eq 'SL1' or wa_abs_attribs-subty eq 'CL1'.

        wa_abs_attribs-beguz = '090000'.
        wa_abs_attribs-enduz = '130000'.
      ELSEIF wa_abs_attribs-subty eq 'SL2' or wa_abs_attribs-subty eq 'CL2' .
         wa_abs_attribs-beguz = '130000'.
        wa_abs_attribs-enduz = '170000'.
      ENDIF.

        modify attabs_deduct_wa-abs_attribs from wa_abs_attribs index 1 TRANSPORTING beguz enduz.
        clear wa_abs_attribs.
